...
function createLogger(level: LogLevel) {
  ...
  return (message: string) => {
    ...
    //Definir el mensaje del log
    const msg = {
      info: `[INFO: ${timeStamp}] ${message}`,
      warn: `[WARNING: ${timeStamp}] ${message}`,
      error: `[ERROR: ${timeStamp}] ${message}`,
    };

    //Definir un objeto de colores para cada nivel
    const color = {
      info: COLORS.white,
      warn: COLORS.yellow,
      error: COLORS.red,
    ...
    return console.log(`%c${msg[level]}`, color[level]);
  };
}

// Ejemplo de uso
function main() {
  // Crear los manejadores de logs
